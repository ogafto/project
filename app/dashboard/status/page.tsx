  ...
  return (
    <main className="min-h-screen bg-[#0E0E11] text-white flex flex-col items-center px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="w-full max-w-5xl flex items-center justify-between pb-8 border-b border-white/[0.08]">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> Wróć do Panelu Głównego
        </Link>
      ...
      <div className="w-full max-w-5xl mt-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-white flex items-center gap-3">
              Diagnostyka Systemowa Panelu
            </h1>
            <p className="mt-2 text-sm text-[#707070]">
              Status 8 głównych usług, baz danych, bramek płatności i kolejek zamówień w czasie rzeczywistym.
            </p>
          </div>
          ...
          {loading ? (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-white">
                    {overall === "operational"
                      ? "Wszystkie usługi platformy są operacyjne"
                      : overall === "warning"
                      ? "Wykryto ostrzeżenia w konfiguracji"
                      : "Wykryto błędy w jednym z filarów"}
                  </h2>
                  <p className="text-xs text-[#A1A1AA] mt-1">
                    {overall === "operational"
                      ? "Baza danych, autoryzacja, Stripe, Resend i silnik sklepów działają z optymalną wydajnością."
                      : "Szczegółowy log błędu znajduje się poniżej w odpowiedniej karcie."}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 text-xs text-[#707070] self-start sm:self-center">
                ...
                Ostatnia weryfikacja: {lastRefreshed ? lastRefreshed.toLocaleTimeString() : "teraz"}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-12 text-center text-xs text-[#505055] pb-12">
          IskraL Platform Engine • Wszystkie testy są przeprowadzane w izolacji na żywo.
        </div>
      </div>
    </main>
  );
